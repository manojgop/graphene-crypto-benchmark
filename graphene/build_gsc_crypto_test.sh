...
# cryptotest docker image name.
IMAGE_NAME=cryptotest
# Graphenized docker image name.
GSC_IMAGE_NAME=gsc-$IMAGE_NAME

# Check if CRYPTO_TEST_HOME is set. If not exit.
if [ -z "$CRYPTO_TEST_HOME" ]; then
  echo "CRYPTO_TEST_HOME is not set. Please set it to crypto test repo top level directory"
  echo "In the current shell: export CRYPTO_TEST_HOME=<repo top level directory>"
  exit
else
  echo "CRYPTO_TEST_HOME is set to $CRYPTO_TEST_HOME"
fi
...
# Manifest files
MANIFEST_FILE_DIR="${CRYPTO_TEST_HOME}/graphene"
MANIFEST_FILES="bash.manifest
openssl.manifest"
